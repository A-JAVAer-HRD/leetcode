...
class Solution {
    public int myAtoi(String s) {
        int res = 0;
        int sign = 0; // 有没有符号 0没有符号 -1负号 1＋
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);

            if (c == ' ' && (i == 0 || (i > 0 && s.charAt(i - 1) == ' '))) { // 只处理前导' '
                continue;
            }
            ...
            if (c >= '0' && c <= '9') {
                ...
                if ((res == Integer.MAX_VALUE / 10 && val >= 8 && sign == -1)) {
                    return Integer.MIN_VALUE;
                } else if (res == Integer.MAX_VALUE / 10 && val >= 7 && sign == 1) {
                    return Integer.MAX_VALUE;
                }
                res = res * 10 + val;
                continue;
