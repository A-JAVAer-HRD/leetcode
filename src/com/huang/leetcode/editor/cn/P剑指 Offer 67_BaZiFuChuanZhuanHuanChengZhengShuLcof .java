...
class BaZiFuChuanZhuanHuanChengZhengShuLcof{
    public static void main(String[] args) {
        //Solution solution = new P剑指 Offer 67_BaZiFuChuanZhuanHuanChengZhengShuLcof().new Solution();
...
class Solution {
    public int strToInt(String str) {
        int res = 0, // 返回值
            bndry = Integer.MAX_VALUE / 10; // 越界标志
        int i = 0, // 遍历
            sign = 1, // 正负标志位
            length = str.length();
        if (length == 0) return 0;
        while (str.charAt(i) == ' ') { // 跳过空格
        ...
        for (int j = i; j < length; j++) {
            ...
            if (res > bndry || res == bndry && str.charAt(j) > '7') // 越界
